...
func _process(delta: float):
	...
	if is_rolling:
		...
		points_pos[0] = visual.global_position + Vector2(cos(angle), sin(angle)) * 6.0
	else: points_pos[0] = pos + visual.global_position
	...
	if is_rolling:
		...
		for i in range(1, length):
			...
			points_pos[i] = points_pos[i].lerp(target_circle_pos, stiffness)
	else:
		for i in range(1, length):
